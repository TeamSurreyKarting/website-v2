			  ...
			  <DropdownMenuTrigger asChild>
				...
				>
				  <div className="grid flex-1 text-left text-sm leading-tight">
					  {user ? (<span className="truncate font-semibold">{user.user_metadata.firstName} {user.user_metadata.lastName}</span>) : <Skeleton />}
					  {user ? (<span className="truncate text-xs">{user.email}</span>) : <Skeleton /> }
				  </div>
