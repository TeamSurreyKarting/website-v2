...
export default function MembershipTypeFilter({ membershipTypes }: { membershipTypes: Database['public']['Tables']['MembershipTypes']['Row'][] }) {
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className="w-[225px] p-0 bg-ts-blue-600 text-white">
				<Command>
					<CommandInput className={"text-gray-100"} placeholder="Search membership types..." />
					<CommandList>
							...
							{membershipTypes.map((membershipType) => (
								<CommandItem
									...
									onSelect={(newMembership) => {
										...
										const isAlreadySelected = selectedMemberships.includes(newMembership);

										if (!isAlreadySelected) {
											setSelectedMemberships([...selectedMemberships, newMembership]);
										} else {
											setSelectedMemberships(selectedMemberships.filter((x) => x !== newMembership));
										}
								...
								>
									...
									<Check
										className={clsx("ml-auto",
											selectedMemberships.includes(membershipType.id) ? "opacity-100" : "opacity-0"
										)}
									/>
								</CommandItem>
							))}
