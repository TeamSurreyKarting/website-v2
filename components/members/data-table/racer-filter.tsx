...
export default function RacerFilter({ racers }: { racers: Database['public']['Tables']['Racers']['Row'][] }) {
			...
			<PopoverTrigger asChild>
				...
				>
					{selectedRacers.length > 0
						? (<span>{selectedRacers.length} Racer{selectedRacers.length === 1 ? '' : 's'} Selected</span>)
								...
								<CommandItem
									...
									onSelect={(racerId) => {
										...
										if (!isAlreadySelected) {
											...
											setSelectedRacers(selectedRacers.filter((x) => x !== racerId));
										}
								...
								>
									...
									<Check
										className={clsx("ml-auto",
											selectedRacers.includes(racer.id) ? "opacity-100" : "opacity-0"
										)}
									/>
								</CommandItem>
							))}
