...
async function getData(membershipTypeIds?: string[], racerIds?: string[]): Promise<MembershipNested[]> {
	const supabase = await createClient();

	// Build query
	const query = supabase.from("Members").select('id, addedAt, addedBy, MembershipTypes!inner( id, name, validFrom, validUntil ), Racers!inner( id, fullName )');
...
export default async function MembersDataTable({ membershipTypeIds, racerIds }: { membershipTypeIds?: string[], racerIds?: string[] }) {
