...
export default function RacerCombobox({ defaultValue, onValueChange }: { defaultValue?: string, onValueChange?(value: string): void; }) {
	let [racers, setRacers] = useState<{id: string, fullName: string}[]>([]);
	let [isLoading, setLoading] = useState<boolean>(false);
	let [filter, setFilter] = useState<string>("");
	let [isOpen, setIsOpen] = useState<boolean>(false);
	let [selectedRacer, setSelectedRacer] = useState<{id: string, fullName: string} | undefined>(undefined);
	...
	useEffect(() => {
		async function getDefaultRacer() {
			...
			const query = supabase.from("Racers").select('id, fullName').eq('id', defaultValue).single();
		...
		<Popover open={isOpen} onOpenChange={setIsOpen}>
			<PopoverTrigger asChild>
				<Button
					variant="outline"
					role="combobox"
					className="w-[200px] justify-between bg-ts-blue-600 text-white"
				>
							...
							<CommandGroup>
								{ racers.map((racer) => (
									<CommandItem
										...
										onSelect={(value) => {
											setSelectedRacer(selectedRacer?.id === value ? undefined : racers.find((r) => r.id === value));
