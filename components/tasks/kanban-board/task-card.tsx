...
export default function TaskCard({ task, authedUserId }: { task: Database['public']['Views']['TaskDetailsView']['Row'], authedUserId: string|undefined }) {
		...
		>
			...
			>
				...
				{ task.primarily_responsible_person_id && (
					<span className={clsx({
						"bg-ts-gold-950": task.primarily_responsible_person_id === authedUserId
					})}>
												<MdSupervisedUserCircle />
						{ task.primarily_responsible_person_id === authedUserId ? "Me" : task.primarily_responsible_person_full_name }
											</span>
				)}
				<span>
											<GoCommentDiscussion />
