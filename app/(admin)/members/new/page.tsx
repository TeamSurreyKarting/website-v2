...
export default async function NewMemberPage(props : {
	...
	async function getMembershipTypes(): Promise<Database['public']['Tables']['MembershipTypes']['Row'][]> {
		...
		const query = supabase.from("MembershipTypes").select().lt("validFrom", now).gt("validUntil", now);
	...
	async function getRacers(): Promise<Database['public']['Tables']['Racers']['Row'][]> {
		const supabase = await createClient();
	...
	return (
		<>
			...
			<AssignMembershipForm memberships={membershipTypes} racers={racers} defaultRacer={defaultRacerInFetchedRacers.length ? defaultRacerInFetchedRacers[0].id : null} />
		</>
	)
}
