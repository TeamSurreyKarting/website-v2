...
async function fetchAccountName(accountId: string): Promise<string | undefined> {
...
export default async function BreadcrumbSlot({
	...
	return (
		<BreadcrumbList>
			...
			<BreadcrumbSeparator />
			<BreadcrumbItem>
				<BreadcrumbLink className={"text-gray-300"} href={`/finances/${accountId}`}>{accountName ?? accountId}</BreadcrumbLink>
			</BreadcrumbItem>
			<BreadcrumbSeparator />
			<BreadcrumbItem>
				<BreadcrumbPage className={"text-white capitalize"}>New Transaction</BreadcrumbPage>
			</BreadcrumbItem>
		</BreadcrumbList>
	);
}
